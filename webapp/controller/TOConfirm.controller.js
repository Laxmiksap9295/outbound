...
sap.ui.define([
	...
	return Controller.extend("LoadingConfirmation.controller.TOConfirm", {
		...
		onInit: function() {
			...
			this.getOwnerComponent().getRouter().getRoute("TOConfirm").attachPatternMatched(this._onObjectMatched, this);
		...
		_LoadDeliveryItems: function(that, vbeln, fSaved) {
			...
			oGetModel.read(oPath, null, null, true, function(oData, oResponse) {
					...
					if (oDataR.DelReturnNav.results["length"] !== 0) {
						...
						sap.ui.core.BusyIndicator.hide();
						var opn_quancheck = sap.ui.getCore().byId("id_OpQty").getValue();;
		...
		onItemConfirm: function() {
			var vOpenQty = sap.ui.getCore().byId("id_OpQty").getValue();
			var that = this;
		...
		onItemPress: function(oEvent) {
			...
			if (that.selectedItem.Lgort !== "" && that.selectedItem.Tanum !== "0000000000" && that.selectedItem.ToConfirm !== "") {
					...
					success: function(oData, Response) {
						...
						if (oData.results.length > 0) {
							if (oData.results[0].F4FieldsNav.results.length > 0) {
								...
								if (!that._ItemDialog) {
									that._ItemDialog = sap.ui.xmlfragment("LoadingConfirmation.fragment.ItemDetailsTO", that);
		...
		// 					var vOpenQty = sap.ui.getCore().byId("id_OpQty").getValue(); //Added by Suvethaa for OpenQty Change on 22.08.2022
		// 					if (vSameBatchUpd) {
		...
		// },
			...
			var vErr = false;
			var vErrMsg = "";
			if( that.selectedItem.Werks !=='2901'){ // Added for Pallet scanning
			if (!vScanMatnr || !vScanPlant || !vScanBatch || !vScanQty || !vScanQtyUom) {
				vErr = true;
				vErrMsg = vErrMsg + self.getView().getModel("i18n").getResourceBundle().getText("InvalidQr") + "\n";
			}
			...
			if (!vErr) {
				...
				that.getView().getModel('odata').read("/ToScanValSet", {
					filters: [new sap.ui.model.Filter("Vbeln", sap.ui.model.FilterOperator.EQ, vVbeln),
						...
						// new sap.ui.model.Filter("InUom", sap.ui.model.FilterOperator.EQ, vScanQtyUom),
						// new sap.ui.model.Filter("OutUom", sap.ui.model.FilterOperator.EQ, that.selectedItem.Vrkme),
						// // new sap.ui.model.Filter("Vbeln", sap.ui.model.FilterOperator.EQ, that.selectedItem.Vbeln),// Added for Pallet Changes
						new sap.ui.model.Filter("Werks", sap.ui.model.FilterOperator.EQ, that.selectedItem.Werks) // Added for Pallet Changes
					],
					urlParameters: {
						$expand: "ToScanValOutNav,DelReturnNav"
					},
						...
						if (oData.results[0].DelReturnNav.results.length == 0) {
							...
							if (!vErr) {
								if (!vSameBatchUpd) {
									...
									var Batchitemnew = {
										// Charg: vScanBatch,// Commented for Pallet Scanning issue
										Charg :oData.results[0].ToScanValOutNav.results[0].Charg,// Added for Pallet Scanning issue
										Bstme: Batchitem.Items[0].Bstme,
										Del_type: Batchitem.Items[0].Del_type,
										Fbatc: Batchitem.Items[0].Fbatc,
										Fpicq: Batchitem.Items[0].Fpicq,
									...
									Batchitem.Items[0].Vrkme = vScanQtyUom;
									Batchitem.Items[0].Lenum = vHU;
									// Batchitem.Items[0].Charg = vScanBatch;// Commented for Pallet Scanning issue
									Batchitem.Items[0].Charg  = oData.results[0].ToScanValOutNav.results[0].Charg,// Added for Pallet Scanning issue
									Batchitem.Items[0].Uecha = "000000";
									// Batchitem.Items[0].Uecha = oGetObject.Posnr;
									// Batchitem.Items[0].Posnr = vSubItemPosnr.toString();
									Batchitem.Items[0].Posnr = oGetObject.Posnr;
		...
		fnColorChange: function() {
			...
			sap.ui.getCore().byId("id_OpQty").setValue(vOpenQty.toFixed(3));
		},
		...
		fnCreateTO: function(oEvent) {
			...
			var txt_tocreat = that.getView().getModel("i18n").getResourceBundle().getText("Crtto", vGetHeaderObj.Posnr);
							...
							oPostModel.create('/DeliverySet', oEntry, null, function(oData, res) {
								...
								if (oReturn.Type === "S") {
									...
									for (var i = 0; i < aHeaderData.length; i++) {
										if (Number(aHeaderData[i].Posnr) === Number(vGetHeaderObj.Posnr)) {
		...
		//Begin of Confirm Tranfer Order			
		fnConfirmTO: function(oEvent) {
			var that = this;
			var txt_tocreat = that.getView().getModel("i18n").getResourceBundle().getText("Cnfto", that.selectedItem.Posnr);
